...
import { HERO, GALLERY } from "@/content/site";
    ...
    <div ref={viewport} className="fixed inset-0 touch-none overflow-hidden bg-background">
      <div ref={track} className="flex h-full w-max items-center will-change-transform">
        ...
        <section className="flex h-full w-screen shrink-0 flex-col items-center justify-center gap-[clamp(1rem,2vw,1.75rem)] px-[clamp(1.5rem,6vw,6rem)] text-center">
          <p className="font-display text-[11px] uppercase tracking-[0.32em] text-cream/70">{HERO.eyebrow}</p>
          <h1 className="font-display text-[clamp(4.5rem,19vw,16rem)] font-black leading-[0.8] tracking-[0.01em] text-cream">{HERO.title}</h1>
          <p className="max-w-[52ch] font-sans text-[12px] uppercase leading-[1.9] tracking-[0.1em] text-cream/65">{HERO.tagline}</p>
          <span className="mt-2 font-display text-[11px] uppercase tracking-[0.32em] text-cream/55">{HERO.cta} →</span>
        </section>
        ...
        {GALLERY.map((p, i) => (
          <div key={i} className="flex h-full shrink-0 items-center pr-[clamp(0.75rem,2vw,1.5rem)]">
            <div className="relative h-[clamp(52svh,70svh,80svh)] bg-white/5" style={{ aspectRatio: `${p.w} / ${p.h}` }}>
              <Image src={p.src} alt={p.alt} fill sizes="80vw" className="object-cover" priority={i < 2} draggable={false} />
            </div>
          </div>
        ))}
